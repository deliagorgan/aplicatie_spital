...
<body>
  ...
  $sql1 = "SELECT 
    ...
    inr.DataInternarii AS 'DataInternare',
    inr.DataExternarii AS 'DataExternare'
FROM 
    tblFiseMedicale fm
JOIN 
...
LEFT JOIN 
  ...
  if ($result->num_rows > 0) {
    ...
    echo "<tr><th class=\"title\" colspan=\"7\">Fise Medicale</th></tr>";
    echo "<tr><th>IdFisa</th><th>Medic</th><th>Pacient</th><th>Rezultat</th><th>DataInternare</th><th>DataExternare</th><th>";

    echo "<button onclick=\"location.href='addMF.php'\">➕Fisa Medicala</button>";
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      echo "<tr><td>" . $row["IdFisa"] . "</td><td>" . $row["Medic"] . "</td><td>" . $row["Pacient"] . "</td><td>" . $row["Rezultat"] . "</td><td>" . $row["DataInternare"] . "</td> <td>" . $row["DataExternare"] . "</td> <td><a href='delete_mf.php?IdFisa=" . $row['IdFisa'] . "' onclick='return confirm(\"Are you sure you want to delete this record?\")'>Delete</a></td></tr>";
    }
    echo "</table>";
  } else {
    echo "0 results";
